...
- **Local models and knowledge bases**: Read [references/local-models.md](references/local-models.md) when listing/downloading/deleting local models or preparing a knowledge base. Default downloads to ModelScope unless the user chooses another source or the catalog does not offer it. Do not set a download timeout; omit `--timeout` unless the user explicitly requests one. If local embedding or reranker models are missing, guide their download, verify completion, then create the knowledge base with the selected provider model IDs.
...
- **[scripts/local_models.py](scripts/local_models.py)**: List local model status with `list --type embedding` or `list --type reranker`; download with `download --type <type> --model-id <id>` (defaults to ModelScope; use `--source huggingface` to override), or delete with `delete --type <type> --model-id <id>`. All commands default to no HTTP timeout. Downloads wait for completion; recheck status before creating a knowledge base.

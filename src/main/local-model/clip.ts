...
export class LocalCLIPModel implements ClipModel {
  ...
  async encodeTexts(texts: string[], options: EncodeTextOptions = {}): Promise<Float32Array[]> {
    const { model, tokenizer, textModel } = await localModelManager.ensureModelLoaded(
    ...
    const inputs = tokenizer(normalizedTexts, {
      padding: true,
      truncation: true,
    });
    ...
    if (textModel) {
      ...
      const textEmbeds = output.text_embeds?.data as Float32Array;
  ...
  async encodeImages(imagePaths: string[]): Promise<Float32Array[]> {
    const { model, tokenizer, processor } = await localModelManager.ensureModelLoaded(
      'zero-shot-image-classification',
      this.modelId,
      this.modelPath
    );
  ...
  private async getImageSize(): Promise<number> {
    const { model, tokenizer, processor } = await localModelManager.ensureModelLoaded(
      'zero-shot-image-classification',
      this.modelId,
      this.modelPath
    ...
    const config = (model as { config?: { vision_config?: { image_size?: number } } }).config;

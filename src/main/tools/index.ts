...
class ToolsManager extends BaseManager {
  ...
  public async importMcp(data: string) {
    ...
    for (const [key, value] of Object.entries(mcpServers)) {
      ...
      const mcpClient = {
        id: id,
        mcp,
        status: 'starting' as McpClientStatus,
        error: undefined,
      };
      const mcpStatus = {
        id: id,
      ...
      await appManager.sendEvent(McpEvent.McpClientUpdated, mcpStatus);
      try {
  ...
  public async toggleToolActive(id: string) {
    if (id.startsWith('claude-skill:')) {
      ...
      await appManager.sendEvent(McpEvent.McpClientUpdated, tool);
      return tool;
    } else {
      const tool = await this.toolsRepository.findOne({ where: { id } });
      if (tool.type == 'mcp') {

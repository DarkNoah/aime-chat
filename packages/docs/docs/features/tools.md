...
长时间运行的 Bash 会话会被记录为后台会话。你可以在聊天界面的 Bash 输出面板中继续查看输出，也可以直接停止正在运行的后台进程。

当聊天绑定到项目时，任务列表和上下文注入会按项目范围汇总后台 Bash 会话，而不是只看当前单个聊天线程。这适合查看整个项目里正在运行的开发服务器、构建任务或巡检命令。
...
如果 Bash 命令仍在运行，可以在聊天页面的 Bash 输出面板或相关任务视图中找到该后台会话，并使用停止按钮结束进程。项目级聊天会显示同一项目范围内的后台 Bash 会话，方便统一处理长时间运行的命令。

...
function main() {
  ...
  const webpackProcesses = processes.filter(
    (processInfo) =>
      isWebpackProcess(processInfo) &&
      startsWithNodeInCurrentWorkingDirectory(processInfo),
  );

  if (webpackProcesses.length === 0) {
    console.log(`未检测到命令行以 "node ${cwd}" 开头的 webpack 相关进程。`);
    return;
  }

  console.log(`检测到 ${webpackProcesses.length} 个 webpack 相关进程：`);
  console.log(formatTable(webpackProcesses));
}

...
- 🌐 **内置浏览器** - 使用 Electron Chromium，共享登录状态，支持每个聊天多个标签页及并发聊天间的操作隔离
...
|------|------|
| 微信 | 接入微信进行 AI 智能对话 |
| Telegram | 集成 Telegram Bot 进行 AI 交互 |
...
- 在 **设置 → 实例管理** 中查看共享 Electron Chromium 的数据目录与标签页数量，或关闭所有浏览器标签页。浏览器数据固定保存在 `<应用用户数据目录>/instances/default_browser`，无需另行安装浏览器。
